...
import { Trophy, Calendar, Users, ArrowRight, Star, ChevronRight } from 'lucide-react'

export default async function FutsalPortalPage() {
    ...
    <div className="min-h-screen bg-light">
      ...
      <header className="bg-dark text-white">
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold font-heading">⚽ Futsal SF Winner</h1>
          ...
          {teams && teams.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {teams.map((team: any) => (
                <Link
                  key={team.id}
                  href={`https://futsal.sfwinner.site/team/${team.slug}`}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md hover:border-primary transition group"
                >
                  ...
                  <h3 className="font-semibold text-dark text-center group-hover:text-primary transition">
                    {team.name}
                  </h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
              <p className="text-gray-500">Belum ada tim futsal. Cek lagi nanti!</p>
            </div>
          )}
        </div>
      </section>
              ...
              {tournaments.map((t: any) => (
                <div key={t.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <Link
                      href={`https://futsal.sfwinner.site/tournament/${t.slug}`}
                      className="text-primary text-sm hover:underline flex items-center gap-1 justify-end mt-1"
                    >
