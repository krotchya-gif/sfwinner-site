...
export default async function TeamPage({ params }: PageProps) {
  ...
  // Get players (public fields only: display_name, photo_url, jersey_number, position, status)
  ...
  const sportColor = (team.sport as any)?.slug === 'basket' ? 'orange'
    : (team.sport as any)?.slug === 'renang' ? 'cyan' : 'primary'
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm flex items-center gap-1">
                  {(team.sport as any)?.name} • {team.branch_location}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-dark font-heading flex items-center gap-2">
              <Users className={`w-6 h-6 text-${sportColor}`} />
              Players
            </h2>
