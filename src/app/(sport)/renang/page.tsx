...
export default async function RenangPortalPage() {
  ...
  // Get recent matches (swimming events treated as "matches")
    ...
    <div className="min-h-screen bg-light">
      ...
      <header className="bg-dark text-white">
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold font-heading">🏊 Renang SF Winner</h1>
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-cyan-500 to-cyan-600 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h2 className="text-4xl md:text-5xl font-bold font-heading mb-4">
            Renang Portal
          </h2>
          <p className="text-lg text-white/80 mb-8 max-w-2xl mx-auto">
            Prestasi renang SF Winner — berani berenang, berani menang
          </p>
          <div className="flex items-center justify-center gap-4">
            <Link href="#teams" className="bg-white text-cyan-600 font-semibold px-6 py-3 rounded-lg hover:bg-gray-100 transition flex items-center gap-2">
          ...
          {teams && teams.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {teams.map((team: any) => (
                <Link
                  key={team.id}
                  href={`https://renang.sfwinner.site/team/${team.slug}`}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md hover:border-cyan-500 transition group"
                >
                  ...
                  <h3 className="font-semibold text-dark text-center group-hover:text-cyan-600 transition">
                    {team.name}
                  </h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
              <p className="text-gray-500">Belum ada tim renang. Cek lagi nanti!</p>
            </div>
          )}
        </div>
      </section>
              ...
              {tournaments.map((t: any) => (
                <div key={t.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <Link
                      href={`https://renang.sfwinner.site/tournament/${t.slug}`}
                      className="text-cyan-500 text-sm hover:underline flex items-center gap-1 justify-end mt-1"
                    >
